...
const GetPokemons = ({ pokemons, setPokemons, initialValues }) => {
  ...
  const pokemonsToShow = shortRoutePokemons?.slice(
    lastIndex - perPage,
    lastIndex
  );
  ...
  const getPokemonsType = (obj) => {
    setPage(1);
    initialValues();
    axios.get(obj).then((res) => setPokemons(res.data));
  };
  ...
  const getLastBtn = () => {
    if (totalPages > 10) {
      if (page < totalPages) {
        return (
          <i
            className="bx bxs-right-arrow-circle bx-md btn_acces"
  ...
  return (
    <div className="pokemons__container">
      ...
      <div className="acces__container">
        {page > 1 && (
          <i
            className="bx bxs-left-arrow-circle bx-md btn_acces"
            onClick={() => setPage(1)}
          ></i>
        )}
        {acces?.map((num) => (
