...
const Item = () => {
  ...
  const types = () => {
    for (let i = 0; i < pokemon.types?.length; i++) {
      pokemonTypes.push(
        `${pokemon.types[i].type.name.charAt(0).toUpperCase()}${pokemon.types[
          i
        ].type.name.slice(1)}`
      );
    }
  ...
  return (
    <div className="item">
      <div className="name__type">
        <h2 style={{ color: getBackground() }}>{`${pokemon.name
          ?.charAt(0)
          .toUpperCase()}${pokemon.name?.slice(1)}`}</h2>
      ...
      >
        <i onClick={() => navigate(-1)} className="bx bxs-chevron-left bx-lg">
          Regresar
        </i>
        ...
        <div className="item_characteristics">
          ...
          <div className="circle">
            <i className="bx bx-circle bx-burst bx-lg"></i>
          </div>
          <p>
            <strong>Peso:</strong>
          </p>
      ...
      <div className="item_description">
        <div className="title_description">
          <h3
            onClick={() => setInfo("habilidades")}
            style={{ backgroundColor: info === "habilidades" && "#3b6e8d" }}
          >
            Habilidades
          </h3>
          <h3
            onClick={() => setInfo("movimientos")}
            style={{ backgroundColor: info === "movimientos" && "#3b6e8d" }}
          >
            Movimientos
          </h3>
        </div>
